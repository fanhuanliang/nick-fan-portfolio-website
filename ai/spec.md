...
10. ⚠️ **Implemented, finalization pending (Phase 4)** — dark mode added in the working tree: CSS custom properties + `.dark` class toggle (not Tailwind `dark:` classes, since no component uses Tailwind utility classes yet — see plan.md Phase 4 execution note), toggle button in Navbar, persisted to localStorage, respects `prefers-color-scheme` on first visit. Do not call this complete until the final browser regression pass and commit are done.
