...
12. ✅ **Fixed (Phase 7)** — accessibility audit completed: skip link and `main#content` landmark added, visible focus states tightened, icon-only controls audited for accessible names, modal dialog semantics/focus/Escape behavior added, sampled light/dark contrast verified, and image `alt` coverage added to regression checks.
